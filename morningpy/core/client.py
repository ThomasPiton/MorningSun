...
class BaseClient:
    ...
    async def get_async(
        ...
        metadata : dict, optional
            Additional metadata to attach to the response (e.g., id_security)
        """
    ...
    async def fetch_all(
        self,
        session: aiohttp.ClientSession,
        requests: List[Tuple[str, dict, dict]],  # Changed to include metadata
    ) -> List[Any]:
        """
        ...
        Parameters
        ----------
        requests : list of (str, dict, dict)
            A list of tuples ``(url, params, metadata)`` specifying endpoints,
            query parameters, and metadata.
        """
        tasks = [
            self.get_async(session, url, params=params, metadata=metadata) 
            for url, params, metadata in requests
        ]
