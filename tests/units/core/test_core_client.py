...
class TestGetAsync:
    ...
    @pytest.mark.asyncio
    async def test_successful_request(self, base_client, mock_response):
        ...
        result = await base_client.get_async(
            mock_session,
    ...
    @pytest.mark.asyncio
    async def test_passes_correct_url(self, base_client, mock_response):
        """Test that correct URL is passed to session.get."""
        mock_session = AsyncMock()
        mock_session.get.return_value.__aenter__.return_value = mock_response
        url = "https://api.example.com/endpoint"
        
        await base_client.get_async(mock_session, url)
        
        call_args = mock_session.get.call_args[0]
        assert call_args[0] == url
    
    @pytest.mark.asyncio
    async def test_passes_headers(self, base_client, mock_response):
        """Test that headers are included in request."""
        mock_session = AsyncMock()
        mock_session.get.return_value.__aenter__.return_value = mock_response
        
        await base_client.get_async(mock_session, "https://api.example.com")
        
        call_kwargs = mock_session.get.call_args[1]
        assert call_kwargs['headers'] == base_client.headers
    
    @pytest.mark.asyncio
    async def test_passes_params(self, base_client, mock_response):
        """Test that query parameters are passed correctly."""
        mock_session = AsyncMock()
        mock_session.get.return_value.__aenter__.return_value = mock_response
        params = {"param1": "value1", "param2": "value2"}
        
        await base_client.get_async(
            mock_session,
            "https://api.example.com",
            params=params
        )
    ...
    @pytest.mark.asyncio
    async def test_uses_default_timeout(self, base_client, mock_response):
        """Test that DEFAULT_TIMEOUT is used."""
        mock_session = AsyncMock()
        mock_session.get.return_value.__aenter__.return_value = mock_response
        ...
        call_kwargs = mock_session.get.call_args[1]
        assert call_kwargs['timeout'] == BaseClient.DEFAULT_TIMEOUT
    
    @pytest.mark.asyncio
    async def test_calls_raise_for_status(self, base_client, mock_response):
    ...
    @pytest.mark.asyncio
    async def test_returns_json_response(self, base_client, mock_response):
        ...
        result = await base_client.get_async(mock_session, "https://api.example.com")
        
        assert result == expected_data
    
    @pytest.mark.asyncio
    async def test_injects_metadata_into_dict_response(self, base_client, mock_response):
        """Test that metadata is injected into dict response."""
        mock_session = AsyncMock()
        mock_session.get.return_value.__aenter__.return_value = mock_response
        mock_response.json.return_value = {"data": "test"}
        metadata = {"id": "security_123"}
    ...
    @pytest.mark.asyncio
    async def test_injects_metadata_into_list_response(self, base_client, mock_response):
        """Test that metadata is injected into first item of list response."""
        mock_session = AsyncMock()
        mock_session.get.return_value.__aenter__.return_value = mock_response
        mock_response.json.return_value = [{"data": "item1"}, {"data": "item2"}]
        metadata = {"id": "security_456"}
    ...
    @pytest.mark.asyncio
    async def test_metadata_not_injected_if_none(self, base_client, mock_response):
        """Test that metadata is not injected when None."""
        mock_session = AsyncMock()
        mock_session.get.return_value.__aenter__.return_value = mock_response
        mock_response.json.return_value = {"data": "test"}
    ...
    @pytest.mark.asyncio
    async def test_preserves_existing_metadata(self, base_client, mock_response):
        """Test that existing metadata is not overwritten."""
        mock_session = AsyncMock()
        mock_session.get.return_value.__aenter__.return_value = mock_response
        mock_response.json.return_value = {"data": "test", "metadata": {"existing": "value"}}
        new_metadata = {"id": "new"}
    ...
    @pytest.mark.asyncio
    async def test_raises_on_http_error(self, base_client):
    ...
    @pytest.mark.asyncio
    async def test_raises_on_timeout(self, base_client):
